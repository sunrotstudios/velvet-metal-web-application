...
export function ServiceConnection({ service }: ServiceConnectionProps) {
  ...
  const handleForceSync = async () => {
    ...
    setIsSyncing(true);
    try {
      logger.info(`Starting force sync for ${service}...`);
      await forceSyncLibrary(user.id, service);
      toast({
        title: "Success",
        description:
          "Library sync started successfully. Check console for progress.",
        duration: 5000,
      });
    } catch (error: any) {
      ...
      toast({
        ...
        description:
          error?.message ||
          "Failed to sync library. Check console for details.",
        variant: "destructive",
        duration: 5000,
      });
    } finally {
